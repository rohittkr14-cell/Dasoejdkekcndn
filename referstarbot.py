...
# Flask app for Fake Port (Hosting 24/7 uptime ke liye)
app = Flask('')
...
# Main Super Admin ID (Ye remove nahi ho sakta)
...
cursor.execute(
    """
INSERT OR IGNORE INTO bot_stats (key, value) VALUES ('total_joins', 0)
"""
)
...
def process_valid_start(message):
  try:
    ...
    if ban_status and ban_status[0] == 1:
      bot.send_message(message.chat.id, "<b>ACCESS DENIED: YOU HAVE BEEN RESTRICTED FROM USING THIS SERVICE.</b>", parse_mode="HTML")
      return
    ...
    if not user:
      ...
      if len(args) > 1 and args[0].startswith("/start ref_"):
        try:
          ...
          # Referral tabhi count hoga jab naya user channel/group join kiya ho
    ...
    welcome_text = (
        "<b>WELCOME TO THE EXCLUSIVE TEDDY GIVEAWAY!</b>\n\n"
...
def add_admin(message):
  ...
  if len(args) != 2:
    bot.send_message(message.chat.id, "<b>USAGE: /addadmin <user_id></b>", parse_mode="HTML")
    return
  try:
    ...
    conn.commit()
    bot.send_message(message.chat.id, f"<b>SUCCESSFULLY ADDED {new_admin_id} AS ADMIN.</b>", parse_mode="HTML")
  except ValueError:
...
def remove_admin(message):
  ...
  args = message.text.split()
  if len(args) != 2:
    bot.send_message(message.chat.id, "<b>USAGE: /removeadmin <user_id></b>", parse_mode="HTML")
    return
  try:
    rem_id = int(args[1])
    if rem_id == SUPER_ADMIN_ID:
      bot.send_message(message.chat.id, "<b>YOU CANNOT REMOVE THE SUPER ADMIN.</b>", parse_mode="HTML")
      return
    local_cursor = conn.cursor()
    local_cursor.execute("DELETE FROM admins WHERE user_id = ?", (rem_id,))
    conn.commit()
    bot.send_message(message.chat.id, f"<b>ADMIN {rem_id} REMOVED SUCCESSFULLY.</b>", parse_mode="HTML")
  except ValueError:
    bot.send_message(message.chat.id, "<b>INVALID USER ID.</b>", parse_mode="HTML")
...
def refund_user(message):
  ...
  args = message.text.split()
  if len(args) != 3:
    bot.send_message(message.chat.id, "<b>USAGE: /refund <user_id> <amount></b>", parse_mode="HTML")
    return
  try:
    ...
    conn.commit()
    bot.send_message(message.chat.id, f"<b>SUCCESSFULLY REFUNDED {amount} POINTS TO USER {target_user_id}.</b>", parse_mode="HTML")
    try:
      bot.send_message(target_user_id, f"<b>YOUR ACCOUNT HAS BEEN REFUNDED WITH {amount} REFERRAL POINTS BY ADMIN.</b>", parse_mode="HTML")
    except:
      pass
  except ValueError:
    bot.send_message(message.chat.id, "<b>INVALID FORMAT OR USER ID.</b>", parse_mode="HTML")
...
def create_promo(message):
  ...
  args = message.text.split()
  if len(args) != 3:
    bot.send_message(message.chat.id, "<b>INVALID FORMAT\nUSAGE: /PROMO <CODE> <AMOUNT></b>", parse_mode="HTML")
    return
  code = args[1]
  try:
    amount = int(args[2])
  except ValueError:
    bot.send_message(message.chat.id, "<b>ERROR: THE AMOUNT MUST BE A VALID NUMBER.</b>", parse_mode="HTML")
    return

  local_cursor = conn.cursor()
  local_cursor.execute("INSERT OR REPLACE INTO promo_codes (code, amount) VALUES (?, ?)", (code, amount))
  conn.commit()
  bot.send_message(message.chat.id, f"<b>PROMO CODE CREATED!</b>\n\nCODE: <code>{code.upper()}</code>\nVALUE: <code>{amount}</code> POINTS", parse_mode="HTML")
...
def broadcast_message(message):
  try:
    ...
    if not text:
      bot.send_message(message.chat.id, "<b>ERROR: PLEASE PROVIDE THE MESSAGE CONTENT YOU WISH TO BROADCAST.</b>", parse_mode="HTML")
      return
    ...
    for (u_id,) in all_users:
      try:
        bot.send_message(u_id, f"<b>IMPORTANT ANNOUNCEMENT:</b>\n\n<b>{text.upper()}</b>", parse_mode="HTML")
        success += 1
      except Exception:
        pass
    bot.send_message(message.chat.id, f"<b>BROADCAST SUCCESSFUL!</b>\nMESSAGE SUCCESSFULLY DELIVERED TO {success} ACTIVE USERS.", parse_mode="HTML")
  except Exception as e:
  ...
  try:
    ...
    if call.data == "back_to_main":
      ...
      text = (
          ...
          "<b>SHARE THIS LINK WITH YOUR NETWORK TO EARN POINTS!</b>\n\n"
          "<i>(IF YOU HAVE COMPLETED 15 REFERRALS, CLICK THE CLAIM BUTTON BELOW)</i>"
      )
      ...
      lb_text = "<b>TOP 5 REFERRERS LEADERBOARD:</b>\n\n"
      for idx, (u_id, r_count) in enumerate(top_users, 1):
        lb_text += f"<b>{idx}. USER ID:</b> <code>{u_id}</code> — <b>{r_count} REFERRALS</b>\n"
        
      lb_text += "\n--------------------\n\n"
      lb_text += "<b>YOUR CURRENT STATUS:</b>\n\n"

      if user_stat:
        ref_count, reward_claimed = user_stat
        needed = max(0, 15 - ref_count)
        lb_text += f"<b>TOTAL REFERRALS:</b> <code>{ref_count} / 15</code>\n"

        if ref_count >= 15:
          if reward_claimed == 0:
            lb_text += "\n<b>TARGET ACHIEVED! YOU ARE ELIGIBLE FOR THE REWARD.</b>"
          else:
            lb_text += "\n<b>STATUS: YOU HAVE ALREADY SUBMITTED YOUR DETAILS FOR VERIFICATION.</b>"
        else:
          lb_text += f"\n<b>PROGRESS: YOU ONLY NEED {needed} MORE REFERRALS TO UNLOCK YOUR REWARD!</b>"
      else:
        lb_text += "<b>PLEASE USE THE /START COMMAND TO INITIALIZE YOUR PROFILE.</b>"
      ...
      edit_msg(call, "<b>REDEEM PROMO CODE</b>\n\n<b>PLEASE ENTER YOUR EXCLUSIVE PROMO CODE BELOW IN THE CHAT:</b>", get_back_markup())
      ...
      edit_msg(call, "<b>CONGRATULATIONS! YOU COMPLETED 15 REFERRALS.</b>\n\n<b>PLEASE TYPE AND SEND YOUR USER ID IN THE CHAT BELOW TO CLAIM YOUR REWARD:</b>", get_back_markup())
...
def handle_user_messages(message):
  ...
  if state == "waiting_for_user_id":
    ...
    local_cursor = conn.cursor()
    # Claim hote hi points minus/reset karne ke liye ref_count ko 0 ya 15 minus kar do
    local_cursor.execute("UPDATE users SET submitted_id = ?, reward_claimed = 1, ref_count = ref_count - 15 WHERE user_id = ?", (text, user_id))
    conn.commit()
    user_state.pop(user_id, None)

    success_msg = "<b>SUBMISSION SUCCESSFUL!</b>\n\n<b>YOUR USER ID HAS BEEN RECORDED AND POINTS HAVE BEEN REDEEMED. OUR ADMINISTRATION TEAM WILL CONTACT YOU SHORTLY.</b>"
    update_bot_message(message.chat.id, user_id, success_msg, get_back_markup())

    username = f"@{message.from_user.username}" if message.from_user.username else "No Username"
    admin_msg = (
        "<b>NEW REWARD CLAIMED!</b>\n\n"
        f"<b>TELEGRAM USER ID:</b> <code>{user_id}</code>\n"
        f"<b>USERNAME:</b> {username}\n\n"
        f"<b>SUBMITTED DETAIL:</b>\n<code>{text}</code>"
    )
    ...
    if promo:
      ...
      try:
        ...
        msg_text = f"<b>SUCCESS!</b>\n\nPROMO CODE APPLIED SUCCESSFULLY. YOU HAVE BEEN REWARDED WITH {amount} REFERRAL POINTS."
        update_bot_message(message.chat.id, user_id, msg_text, get_back_markup())
      except sqlite3.IntegrityError:
        update_bot_message(message.chat.id, user_id, "<b>NOTICE: YOU HAVE ALREADY UTILIZED THIS PROMO CODE.</b>", get_back_markup())
    else:
      update_bot_message(message.chat.id, user_id, "<b>INVALID CODE: THE PROMO CODE YOU ENTERED DOES NOT EXIST OR HAS EXPIRED.</b>", get_back_markup())
...
if __name__ == "__main__":
  keep_alive()
  print("Bot is successfully running with updated rules...")
